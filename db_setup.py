...
def update_year_entries():
	...
	for year_entry in years:
		...
		popular_genre_query = db.session.query(Genre, func.count(Genre.genre_id).label("count"), Genre.genre_id).join(get_genre_table()).join(Game).filter(Game.release_year == year_entry.year_id).group_by(Genre).order_by("count DESC").all()
